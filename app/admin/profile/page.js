...
export default function AdminProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (data.success && data.user) {
        ...
        setNameData({ name: data.user.name || '' });
      }
    } catch (err) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-400">Đang tải thông tin cá nhân...</div>;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
          ...
          <form onSubmit={handleUpdateName} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-gray-600 uppercase mb-1">Email (Định danh)</label>
              <input
                type="email"
                disabled
                value={profile.email}
              ...
              />
              <span className="text-[11px] text-gray-400 mt-1 block">Email đăng nhập hệ thống không thể thay đổi trực tiếp tại đây.</span>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition disabled:opacity-50"
            >
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
          ...
          <form onSubmit={handleChangePassword} className="space-y-3">
            ...
            <button
              ...
              className="w-full py-2.5 bg-gray-900 hover:bg-black text-white rounded-xl text-xs font-semibold transition disabled:opacity-50 mt-2"
            >

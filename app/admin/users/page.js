      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100 text-sm">
            {loading ? (
              <tr><td colSpan="5" className="p-6 text-center text-gray-400">Đang tải dữ liệu...</td></tr>
            ) : users.length === 0 ? (
                ...
                <tr key={u.id} className="hover:bg-gray-50/50 transition">
                    ...
                    <div className="flex flex-wrap gap-1.5 max-w-md">
                      {u.permissions && u.permissions.length > 0 ? (
                        u.permissions.map((p) => {
                          ...
                          return (
                            <span key={p} className="bg-blue-50 text-blue-700 text-xs font-medium px-2.5 py-1 rounded-lg border border-blue-100">
                              {matched ? matched.label : p}
                            </span>
                          );
                        })
                      ) : (
                        <span className="text-gray-400 italic text-xs">Chưa được cấp quyền nào</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-right space-x-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 rounded-lg text-xs font-semibold transition"
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-600 uppercase mb-1">Email Đăng Nhập</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-gray-600 uppercase mb-2">Phân Quyền Truy Cập Module</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-semibold transition disabled:opacity-50"
                >

                ...
                {slides.length === 0 ? (
                  ...
                  slides.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="p-3">
                        {item.image ? (
                          <img src={item.image} alt={item.title || 'Banner'} className="w-20 h-10 object-cover rounded-lg" />
                        ) : (
                          <div className="w-20 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 text-[10px]">Không ảnh</div>
                        )}
                      </td>
                      <td className="p-3 font-bold text-gray-800">{item.title || '---'}</td>
                      <td className="p-3 font-mono text-gray-600">{item.order}</td>
                      <td className="p-3">
                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${item.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>

    ...
    <footer className="bg-[#12412C] text-[#FFFBF3] pt-12 pb-8 border-t border-gray-800">
      <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
        <div>
          ...
          <p className="text-xs">📍 <strong>Địa chỉ:</strong> {settings.address}</p>
          <p className="text-xs mt-1">📞 <strong>Hotline:</strong> <a href={`tel:${settings.hotline}`} className="text-blue-400 font-bold">{settings.hotline}</a></p>
        </div>
        <div>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-6 border-t border-gray-800 text-center text-xs text-gray-500">
